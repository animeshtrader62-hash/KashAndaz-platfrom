...
from app.services.admin.service import block_user, unblock_user, get_risk_flags

router = APIRouter(prefix="/admin", tags=["admin"])


@router.post("/users/{user_id}/block", response_model=dict)
def block(user_id: str, db: Session = Depends(get_db), admin=Depends(require_super_admin)):
...
def unblock(user_id: str, db: Session = Depends(get_db), admin=Depends(require_super_admin)):
    user = db.query(User).filter(User.id == user_id).first()
    if not user:
        raise HTTPException(status_code=404, detail="User not found")
...
def risk_flags(user_id: str, db: Session = Depends(get_db), _=Depends(require_super_admin)):
